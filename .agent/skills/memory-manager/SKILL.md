...
> Всі дозволені вузли, зв'язки та їхні обов'язкові атрибути жорстко задокументовані у файлі `c:\Antigarvity_workspace\.agent\skills\memory-manager\grynya-schema.md`. При використанні MCP інструментів ти **повинен узгодити** поля та зв'язки з цим документом.

Твої аргументи до викликів інструментів повинні повністю дублювати структуру описану у правилі, зокрема враховувати атрибути зв'язків.
...
> [!CAUTION]
> **АБСОЛЮТНА ЗАБОРОНА НА ЗАПИС ЧЕРЕЗ CYPHER**
> Інструмент `mcp_falkordb_query_graph` **КАТЕГОРИЧНО ЗАБОРОНЕНО** використовувати для запису даних (операції CREATE, MERGE, SET, DELETE тощо). Це бекдор, який знищує серверну валідацію через моделі Pydantic в MCP-сервері та порушує архітектуру. Будь-який запис, редагування чи створення зв'язків має відбуватися **ВИКЛЮЧНО** через відповідні інструменти (`mcp_falkordb_add_node`, `mcp_falkordb_link_nodes` тощо).

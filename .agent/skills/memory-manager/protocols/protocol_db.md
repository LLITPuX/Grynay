...
**Семантика:** `/db` відкриває нову сесію у графі **Grynya** (production). Запис завжди в Grynya. **Grynya_v2.0** — для рефакторингу схеми (копія Cursa4); див. `memory-manager/graphs.md`.

## Твої дії (Взаємодія з MCP)

Щоб відкрити сесію, ти маєш використати нативні інструменти (tools) MCP-сервера усі операції запису виконуються в графі **Grynya**.
...
2. Новий `session_ID` має бути суворо послідовним та відповідати шаблону `session_XXX` (де XXX — тризначне число, наприклад, `session_005` → `session_006`). Це забезпечує легку навігацію та структурну чистоту.

...
5.  **Сутності (`:Entity`):** Зв'яжи Analysis з **УСІМА** сутностями, що вже існують у цій сесії (через `[:MENTIONS]`). Якщо є фідбек (крок 2) — витягни з нього нові сутності та створи їх.
...
5.  **Оновлення Сесії в Базі:** Після створення файлу, зафіксуй його шлях у графі:
    ```powershell
    docker exec falkordb redis-cli GRAPH.QUERY Grynya "MATCH (s:Session {id: '<session_id>'}) SET s.file_path = '<повний шлях до файлу>'"
    ```

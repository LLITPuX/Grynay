...
> **Чи активна сесія memory-manager?** Інтерв'ю — це значуща робота, результати якої ОБОВ'ЯЗКОВО треба зберігати. Якщо сесія не активна — запропонуй користувачу розпочати її (`/db`).
...
## Фаза 7: Організація "Передачі" проекту в розробку (Handoff)
...
Варіанти для вибору:
- [Написати План Розробки] → Викликаємо навичку `project-planner` зі створеною специфікацією як вхідними даними.
...
> **Пам'ятка:** Після завершення інтерв'ю запропонуй користувачу зберегти результати сесії (`/sa` або `/ss`) через навичку `memory-manager`.
